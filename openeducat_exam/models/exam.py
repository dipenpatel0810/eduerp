...
class OpExam(models.Model):
    ...
    def verify(self):
        self.subject_ids = [i.id for i in self.session_id.course_id.subject_ids]
        self.subject_ids.extend([i.id for i in self.session_id.course_id.elective_ids])
        return {'domain':{'subject_id':[('id','in',self.subject_ids)]}}
    @api.multi
    def attendees_wizard(self):
        return {    
            'type': 'ir.actions.act_window',
            'res_model': 'op.exam.getattendees',
            'view_type': 'form',
            'view_mode': 'form',
            'view_id': False,
            'target': 'new',
