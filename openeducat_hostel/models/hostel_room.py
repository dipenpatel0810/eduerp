...
class OpHostelRoomAllocation(models.Model):
    ...
    name = fields.Many2one('op.room', 'Room Number', required=True)
    student_ids = fields.Many2many('op.student', string='Allocated Students')
  ...
  #  hostel_display1 = fields.Many2many('op.student',string="Hostel::")


    @api.constrains('students_per_room')
    def check_capacity(self):
        if self.students_per_room <= 0:
            raise ValidationError(_("Enter proper Student Per Room"))
    ...
    def _check_student_capacity(self):
        if len(self.student_ids) > self.students_per_room:
            raise ValidationError(_('Room capacity Over'))
